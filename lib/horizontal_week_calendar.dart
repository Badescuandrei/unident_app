...
class _HorizontalWeekCalendarState extends State<HorizontalWeekCalendar> {
  ...
  getMoreNextWeeks() {
    ...
    for (int index = 0; index < 7; index++) {
      DateTime minusDate = startFrom.add(Duration(days: (index + 7)));
      plus7Days.add(minusDate);
    }
    listOfWeeks.add(plus7Days.toList());
    setState(() {});
  }

  getMoreNextWeeksForSlide() {
    List<DateTime> minus7Days = [];
    DateTime startFrom = listOfWeeks.isEmpty
        ? DateTime.now()
        : listOfWeeks[currentWeekIndex].isEmpty
    ...
    listOfWeeks.add(minus7Days.toList());
    setState(() {});
  }
  ...
  isNextDisabled() {
    return listOfWeeks[currentWeekIndex].last.isAfter(DateTime.now());
  }
                            ...
                            for (int weekIndex = 0; weekIndex < listOfWeeks[ind].length; weekIndex++)
                              Expanded(
                                child: GestureDetector(
                                  onTap: listOfWeeks[ind][weekIndex].isAfter(DateTime.now())
                                      ? () {
                                          onDateSelect(
